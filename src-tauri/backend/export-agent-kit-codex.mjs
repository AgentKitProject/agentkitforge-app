...
const [, , kitPath, destinationSkillsDir, force] = process.argv;

if (!kitPath || !destinationSkillsDir) {
  console.error("Usage: node export-agent-kit-codex.mjs <kitPath> <destinationSkillsDir> <force>");
  process.exit(2);
}

try {
  const core = await loadCore();
  const result = await core.exportAgentKitToCodex(kitPath, destinationSkillsDir, {
    force: force === "true",
  });
